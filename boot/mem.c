...
void print_memmap(struct MemoryMap *map)
{
	...
	for (i = 0; i < map->memmap_desc_entry; i++)
	{
		...
		puts(L"\r\n");
		text_gen(buffer, sizeof(buffer), "%u, %16x, %02lx, %-ls, %16x, %s, %16x, %s %16x, %s, %16x \n", i, (UINT64)desc, desc->Type, get_memtype_name(desc->Type), desc->PhysicalStart, ",", desc->VirtualStart, ",",  desc->NumberOfPages,",", desc->Attribute);
		custom_printf("%s\n", buffer);
