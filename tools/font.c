...
void printFont(char font[256][16]) {
...
void convert8x16To16x16(char font[256][16]) {
    char newFont[256][16];
    for (int col = 0; col < 16; ++col) {
        for (int row = 0; row < 16; ++row) {
            newFont[col][row] = font[(col * 2) % 16][row / 2];  // 8x16から16x16への変換
        }
    }
    // コピーを元の配列に戻す
    for (int col = 0; col < 16; ++col) {
        for (int row = 0; row < 16; ++row) {
            font[col][row] = newFont[col][row];
        }
    }
}
...
int main() {
    ...
    convert8x16To16x16(font);

    printf("\nFont after converting 8x16 to 16x16:\n");
    printFont(font);
